...
class EllipticalBox(inkex.Effect):
    ...
    def effect(self):
        ...
        if min(self.options.height, self.options.width, self.options.depth) == 0:
            inkex.errormsg(u'Erreur: Les dimensions doivent \xEAtre non nulles'.encode("utf-8"))
            error = True

        if self.options.cut_nr < 1:
            inkex.errormsg(u'Erreur: Le nombre de coupes doit \xEAtre d\'au moins 1'.encode("utf-8"))
            error = True

        if (self.options.centralRibLid or self.options.centralRibBody) and self.options.cut_nr % 2 == 1:
            inkex.errormsg('Erreur: La nervure centrale n\'est valide qu\'avec un nombre pair de coupes')
            error = True

        if self.options.unit not in self.knownUnits:
            inkex.errormsg(u'Erreur: Unit\xE9 inconnue. '.encode("utf-8")+ self.options.unit)
            error = True
        ...
        if self.options.centralRibLid or self.options.centralRibBody:
            ...
            draw_SVG_text(innerRibCenter, 'inside', innerRibGrp)
            draw_SVG_text(outerRibCenter, 'outside', outerRibGrp)
